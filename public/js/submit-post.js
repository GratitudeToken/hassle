...
export const submitPost = () => {
    ...
    $('#post-form').addEventListener('submit', (event) => {
        ...
        if (membership === true) {
            ...
            formData.append("type", $('input[name="get-type"]:checked').value);
    ...
    $('#select-type').addEventListener('change', (event) => {
        postType = $('input[name="get-type"]:checked').value
        pollOptions = []
        $$('.voteInput').forEach((el, i) => {
            pollOptions.push(el.value)
        })

        if (postType === 'get-crowd-funded') {
            $('.options').style = ''

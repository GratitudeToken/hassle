...
export const postActions = (queryURL, clearItems, fetchy, looper, populatePosts, charts, voteBTNlisteners, deleteBTNs, removeLastItem) => {
    ...
    filter = localStorage.getItem('filter') || 'day-2-day'

    if (filter !== null) {
        $('#select-type input[name="get-type"]:checked').value = filter
    } else {
        $('#select-type input[name="get-type"]:checked').value = 'day-2-day'
        localStorage.removeItem('filter')
    }

    $('#select-type input[name="get-type"]:checked').addEventListener('change', (e) => {
        localStorage.setItem('filter', $('#select-type input[name="get-type"]:checked').value)
        location.reload()
    })
